...
def get_tool_definitions(enable_markdown_export: bool = True) -> Dict[str, Dict[str, Any]]:
    """Get tool definitions with conditional features applied.
    ...
    # Helper to conditionally add markdown text
    ...
    # All 15 tools are now defined
    
    return tools


def compare_tool_definitions(cached_tool: Dict[str, Any], backend_tool: Dict[str, Any]) -> list[str]:
    """Compare cached tool definition with backend tool definition.
    
    Args:
        cached_tool: Cached tool definition
        backend_tool: Tool definition from backend
    
    Returns:
        List of differences found (empty if no differences)
    """
    differences = []
    tool_name = backend_tool.get("name", "unknown")
    
    # Compare name (should always match, but check anyway)
    if cached_tool.get("name") != backend_tool.get("name"):
        diff = f"Name mismatch: cached='{cached_tool.get('name')}', backend='{backend_tool.get('name')}'"
        differences.append(diff)
    
    # Compare description
    cached_desc = cached_tool.get("description", "")
    backend_desc = backend_tool.get("description", "")
    if cached_desc != backend_desc:
        # Truncate if too long for logging
        desc_preview_len = 100
        cached_preview = cached_desc[:desc_preview_len] + "..." if len(cached_desc) > desc_preview_len else cached_desc
        backend_preview = backend_desc[:desc_preview_len] + "..." if len(backend_desc) > desc_preview_len else backend_desc
        diff = f"Description mismatch: cached='{cached_preview}', backend='{backend_preview}'"
        differences.append(diff)
    
    # Compare inputSchema (simplified - could do deep comparison)
    cached_input = json.dumps(cached_tool.get("inputSchema", {}), sort_keys=True)
    backend_input = json.dumps(backend_tool.get("inputSchema", {}), sort_keys=True)
    if cached_input != backend_input:
        diff = "InputSchema mismatch detected"
        differences.append(diff)
    
    # Compare outputSchema (if present)
    cached_output = json.dumps(cached_tool.get("outputSchema", {}), sort_keys=True)
    backend_output = json.dumps(backend_tool.get("outputSchema", {}), sort_keys=True)
    if cached_output != backend_output:
        diff = "OutputSchema mismatch detected"
        differences.append(diff)
